...
const snake = {
    ...
    afficher:()=>{
        ...
        for (let i of snake.corps){
            contexte.fillRect(i.x, i.y, snake.tete.largeur-2, snake.tete.hauteur-2);
        }
    },
...
function game() {
    gameStarted=true;
    //Déplacer le snake
    if(snake.position.x >= canvas.width || snake.position >= canvas.height || snake.position.x < 0 || snake.position.y < 0){
        alert("LOSER!!!!!");
        gameStarted=false;
        reinitialiser();
    ...
    if (snake.position.x === pomme.position.x && snake.position.y === pomme.position.y) {
        console.log("le snakezer a graille une pommezer")
